...
- [x] **Step 2: Put the design on #4 (durable decision)**
...
- [x] **Step 3: Denylist-scan the two docs without touching the index**
...
- [x] **Step 1: RED — the version check must know every workspace member**
...
- [x] **Step 3: GREEN — `CRATES` gets the new crate**
...
- [x] **Step 1: Write the module with its tests**
...
- [x] **Step 2: Format and commit** (the tests run in CI at Task 12)
...
- [x] **Step 1: Private extraction tool (ops repo)**
...
- [x] **Step 3: Denylist-check the site EQ file, format, commit**
...
- [x] **Step 2: `lib.rs` (final) and the binary**
...
- [x] **Step 3: RED then GREEN — `scripts/golden/check_bundle.py`**
...
- [x] **Step 1: RED — the integrity scan must cover `.psm1`**
...
- [x] **Step 2: `scripts/golden/GoldenPc.psm1`**
...
- [x] **Step 3: `scripts/golden/golden-task.ps1`**
...
- [x] **Step 4: `scripts/golden/Test-GoldenPc.ps1` (runs on the CI `windows` job)**
...
- [x] **Step 1: RED — tests for the pure parts**
...
- [x] **Step 2: GREEN — `scripts/golden/golden_window.py`**
...
- [x] **Step 4: Commit (public parts only)**
...
- [x] **Step 1: RED — tests (`scripts/golden/test_analyze.py`, numpy)**
...
- [x] **Step 2: GREEN — `scripts/golden/analyze.py`**
...
- [x] **Step 3: Goldens size gate in `check_integrity.py` (RED → GREEN)**
...
- [x] **Step 4: `.gitattributes`, playbook rule, router**
...
- [x] **Step 1: Push `dev` and wait for every job** (load the `ci-push-discipline` skill first)
...
- [x] **Step 2: Download the bundle of that exact commit**
...
- [x] **Step 1: Open the window and set up**
...
- [x] **Step 2: Quiet check, save, quit, app stop**
...
- [x] **Step 4: Render only the calibration projects**
...
- [x] **Step 5: Exercise `close-render` once, restore, bring back**
...
- [x] **Step 1: Same opening as window 1 (no setup, no audiocfg)**
...
- [x] **Step 2: Render everything, fetch**
...
- [x] **Step 3: Restore, bring back, report**
...
- [x] **Step 1: Analyse into the repository**
...
- [x] **Step 2: Record every law on #4 (durable)**
...
- [x] **Step 3: Denylist-scan and commit the goldens**
...
- [x] **Step 4: Results pointer in the design note**
...
Windows `20260926T084319Z` (cal) and `20260926T091333Z` (full) ran in dev time with REAPER already saved and quit and the predecessor app kept running (its only graceful exit is the tray menu; no remote-desktop MCP in the session). Open boxes above are the deviations:

- **Task 10 Step 3 / Task 13 Step 3 (MCP, `-audiocfg`):** not done. `new --keep-app` skips the tray Exit, the app's trees are `PC_VOLATILE_TREES` (best effort, never restored), and bring-back never restarts it. Render-instance dialogs were read and closed with their own OK button from a temporary Interactive task (window list + screenshot), removed afterwards. The Dummy Audio `mode` value stays unverified; every render recorded its audio modules and none loaded the ASIO module.
- **Found by running the procedure:** `RENDER_STEMS 2` renders nothing in REAPER 7.65 (now 1, stems + master mix); a fresh render folder scans every default VST3 folder (the scan cache is now copied); the task principal needs the token's own account name over ssh; `@()` around `return ,$array` functions nested the ASIO holder list (false alarm on every render); eq case names now carry the rate.
- **Task 15 Step 4:** the A5/A10 spec change was folded onto #5 (S2) instead of a new ticket.
- **Task 16:** not in this run (push to `dev` only).
