...
- **Windows (`windows` CI job runs `cargo test -p iem-migrate`):** sync a file only through a handle with write access (`File::create`, then `set_permissions`); test helpers that key a directory walk by relative path replace `\` with `/`.
