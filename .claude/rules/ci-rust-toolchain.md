...
- **Mutation gate:** `.cargo/mutants.toml` (profile `mutants`, nextest, excludes with reasons); PR-only, diff-scoped, each shard ≤ 20 min. Shards are 0-based (`--shard k/n`, `n = strategy.job-total`); the shard count is the length of the `shard:` matrix in `ci.yml`. The `mutation-warmup` job builds the `mutants` profile once and saves the dependency cache (`shared-key: mutation`) that every shard restores; shards run `--copy-target=true` after a no-op warm-up so each mutant rebuilds only workspace crates. `mutants-list` (every `dev` push) counts the next PR's diff-scoped mutants and fails when they need more shards than the matrix has at `MUTANTS_PER_SHARD` — resize the matrix before opening the PR. An overrun is a setup bug: add shards or narrow scope (fallback: `--in-place --jobs 1` with more shards), never raise the timeout. Both mutation steps set `NEXTEST_PROFILE=mutants` (`.config/nextest.toml`: fail fast and end running tests at the first failure, and end any test after 10 s as failed, so a mutant that hangs every test is CAUGHT even when the hung tests starve the bounded ones of test threads) and pass `--timeout 60` (with `--baseline=skip` cargo-mutants otherwise allows 300 s per mutant); a test that awaits a lock, semaphore or queue wraps it in `tokio::time::timeout` so a mutant that blocks fails fast (CAUGHT) instead of TIMEOUT. UI survivors: move pure logic into helpers with native tests; exclude only browser-only functions (web_sys, WebSocket/JS closures, Leptos component bodies) per function in `exclude_re`, with a reason. cargo-mutants 27.1 does not apply `exclude_re` to "delete field … from struct … expression" mutants — kill those with a test. `crates/iem-core/build.rs` and `crates/iem-tray/build.rs` key their rerun on `GITHUB_SHA` when there is no `.git` (cargo-mutants' scratch copy), so `BUILD_TIME` does not force a full rebuild per mutant. The full sweep is `mutation-full.yml` (on demand).
