...
- **PINs** are argon2id PHC strings (m=19456 KiB, t=2, p=1) keyed with a 32-byte pepper (`Argon2::new_with_secret`). `pin_hashes.json` holds only `$argon2id$` values — anything else is a load error. The pepper is DPAPI-protected on Windows (`pepper.dpapi`), a plain owner-only test file elsewhere (Linux is test-only). An unreadable pepper is an error, never regenerated (that would void every PIN); a missing pepper is created only while `pin_hashes.json` holds no hash — next to stored hashes it is an error too (recovery: restore the pepper, or move `pin_hashes.json` aside and re-provision with `iem-server pin set-engineer`). Backups never contain PINs.
- **Imported PINs (S4):** `pin_hashes.json` has an additive `imported` list of owners (member ids, `engineer`) whose hash came from the predecessor (`import_*_hash`, via `band_import::import_pins`); `set_member_hash` / `set_engineer_hash` drop the mark, and an import never replaces an unmarked hash — a later import never overwrites an iemmixer-set PIN (program spec §3.4). The predecessor's default PINs come only from a private file at migration time, never from code.
...
- **Same-origin only:** no CORS layer (`app_router` in `lib.rs`); the UI, the tray window and the E2E suite all load the UI from the server. Never add `CorsLayer::permissive()` back.
...
- **Keep these tests:** limiter tables, forged header from a non-loopback peer, IPv6 keyed per /64, the intended LAN-origin spacing, in-flight attempts bounded by the gate, bounded memory, gate cancellation, plaintext store rejected, pepper never regenerated (also not when missing next to PIN hashes), start-up refuses a bad or missing pepper or a bad PIN store, a foreign origin gets no CORS grant, the raw REAPER passthrough stays gone, the CLI never reads the PIN from argv.
